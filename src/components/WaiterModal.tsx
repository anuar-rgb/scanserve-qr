...
import { X, Bell, Lock, ChevronRight, TableProperties, CheckCircle2, Loader2 } from "lucide-react";
...
const T: Record<string, Record<Lang, string>> = {
  ...
  send:            { en: "Call Waiter",              ru: "Вызвать официанта",         kz: "Даяшыны шақыру"                },
  sending:         { en: "Sending...",               ru: "Отправка...",               kz: "Жіберілуде..."                 },
  chooseAction:    { en: "Choose a request",         ru: "Выберите запрос",           kz: "Сұрауды таңдаңыз"              },
  successTitle:    { en: "Waiter Called!",           ru: "Официант вызван!",          kz: "Даяшы шақырылды!"              },
  successDesc:     { en: "Your waiter will be with you shortly.", ru: "Официант подойдёт к вам в ближайшее время.", kz: "Даяшы сізге жақын арада келеді." },
  errorDesc:       { en: "Something went wrong. Please try again.", ru: "Ошибка отправки. Попробуйте ещё раз.", kz: "Қате орын алды. Қайталап көріңіз." },
...
export function WaiterModal({
  ...
  const canSend = tableNumber.trim().length > 0
    && action !== null
    && (action !== "other" || customText.trim().length > 0)
    && sendState === "idle";
  ...
  const handleSend = async () => {
    ...
    const actionValue = action === "other" ? customText.trim() : action;

    const { error } = await supabase
      .from(DB_TABLES.waiterCalls)
      .insert({
        restaurant_id: RESTAURANT_ID,
        table_label:   tableNumber.trim(),
        action:        actionValue,
        status:        "pending",
      });
      ...
      >
        ...
        {sendState === "success" ? (
          <div style={{
            ...
            padding: `${SP.md}px ${SP.md}px ${SP.xl}px`,
            gap: SP.md,
          }}>
            <div style={{
            ...
            <p style={{ fontSize: 20, fontWeight: 700, margin: 0, textAlign: "center" }}>
              {tn("successTitle", lang)}
            </p>
            <p style={{ fontSize: 14, color: muted, margin: 0, textAlign: "center", lineHeight: 1.5 }}>
              {tn("successDesc", lang)}
            </p>
            <button
              onClick={handleClose}
              style={{ ...primaryBtn(false), width: "auto", padding: "12px 32px", marginTop: SP.sm }}
            >
              {tn("close", lang)}
            </button>
          </div>
          ...
          <>
            <div style={{ flex: 1, overflowY: "auto", padding: `${SP.sm}px ${SP.md}px ${SP.lg}px` }}>
              ...
              <div style={{ marginBottom: SP.lg }}>
                ...
                {isTableLocked ? (
                  <div style={{
                    ...
                    padding: "13px 14px", background: surface,
                    border: `1.5px solid ${border}`, borderRadius: R.md,
                  }}>
                  ...
                  <>
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: R.md, color: tableNumber ? textClr : muted,
                        fontSize: 15, cursor: "pointer", textAlign: "left",
                        boxSizing: "border-box", fontFamily: "inherit",
                        fontWeight: tableNumber ? 600 : 400, transition: "border-color 0.15s",
                      } as React.CSSProperties}
                    >
                ...
                <label style={{ display: "block", marginBottom: SP.lg }}>
                  <textarea
                    ...
                    style={{
                      ...
                      borderRadius: R.md, color: textClr, fontSize: 15,
                      outline: "none", boxSizing: "border-box",
                      resize: "none", transition: "border-color 0.15s", fontFamily: "inherit",
                    } as React.CSSProperties}
                  />
                </label>
              )}
              ...
              {/* Error state */}
              {sendState === "error" && (
                <p style={{ fontSize: 13, color: "#E05555", textAlign: "center", margin: `0 0 ${SP.sm}px` }}>
                  {tn("errorDesc", lang)}
                </p>
              )}
            </div>
            ...
            <div style={{ padding: SP.md, borderTop: `1px solid ${border}`, flexShrink: 0 }}>
              <button onClick={handleSend} disabled={!canSend} style={primaryBtn(!canSend)}>
                {sendState === "sending"
                  ? <><Loader2 size={16} style={{ animation: "spin 1s linear infinite" }} /> {tn("sending", lang)}</>
                  : tn("send", lang)
                }
              </button>
            </div>
        ...
        <>
          ...
          >
            ...
            <div style={{
              ...
              gap: 10, padding: `${SP.sm}px ${SP.md}px ${SP.xl}px`,
            }}>
              {tables!.map(t => {
