...
// Validates that a push subscription endpoint is a legitimate HTTPS URL
// and not pointing at internal/cloud-metadata addresses (SSRF prevention).
function isValidPushEndpoint(endpoint: unknown): boolean {
  ...
  // Block loopback, private subnets, and cloud metadata endpoints
  if (
    h === "localhost" ||
    h === "::1" ||
    h.startsWith("127.") ||
