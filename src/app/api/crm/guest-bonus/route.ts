...
export async function GET(req: NextRequest) {
  ...
  // Branch-aware: client passes restaurantId explicitly; fall back to cookie
  const sessionRid =
    searchParams.get("restaurantId") ??
    req.cookies.get("admin_restaurant_id")?.value;
...
export async function PUT(req: NextRequest) {
  ...
  // Branch-aware: prefer body.restaurantId (sent by client) over cookie
  const putRid =
    body.restaurantId ??
    req.cookies.get("admin_restaurant_id")?.value;
